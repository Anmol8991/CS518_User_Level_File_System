...
static int recursive_remove_directory(struct inode *dir_inode) {
    // Base case: If the directory has no subdirectories, or it's a file, return
    if (dir_inode->type == TYPE_FILE || dir_inode->link == 2) {
        return 0;
    }

    // Recursive case: Remove subdirectories first
    for (int i = 0; i < 16; ++i) {
        ...
        int retstat = 0;
            char curr_dir_block[BLOCK_SIZE];
            // Read the directory's data block
            retstat = bio_read(dir_inode->direct_ptr[i], curr_dir_block);
            if (retstat < 0) {
                return retstat;
            }
            ...
            while (i < BLOCK_SIZE / sizeof(struct dirent)){
                ...
                int lookup_result = readi(cur_dirent->ino, &sub_dir_inode);
                ...
                int result = recursive_remove_directory(&sub_dir_inode);
    ...
    // Clear data block bitmap of the target directory
    for (int i = 0; i < 16; ++i) {
        if (dir_inode->direct_ptr[i] != 0) {
            unset_bitmap(db_bitmap_buffer, dir_inode->direct_ptr[i]);
            dir_inode->direct_ptr[i] = 0;
        }
    }

    // Clear inode bitmap and its data block
    unset_bitmap(inode_bitmap_buffer, dir_inode->ino);
...
static int rufs_rmdir(const char *path) {
    ...
    int result = recursive_remove_directory(&target_dir_inode);
    if (result < 0) {
        return result;
    }
    ...
    result = dir_remove(parent_dir_inode, target_name, strlen(target_name));
    if (result < 0){
        return result;
    }
	return 0;
